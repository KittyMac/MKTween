...
public struct MKTweenTiming {
	...
	static public var ElasticInOut: MKTweenTimingFunction = { (time: Double, b: Double, c: Double, d: Double) -> Double in
		...
		if (t < 1) { t -= 1; return -0.5 * (a * pow(2, 10 * t) * sin( (t * d - s) * (2 * Double.pi) / p )) + b }
		t -= 1
